...
\textbf{Nota importante:} dalla legge di Joule si evince che, facendo passare corrente in un resistore, questo dissipa energia, quindi si scalda\footnote{Lo stesso principio è alla base del funzionamento dei fusibili. Il fusibile è un dispositivo di sicurezza che funziona... fondendosi, disattivando il circuito quando l'intensità di corrente supera una certa soglia.}. In laboratorio è importante prestare la massima attenzione per evitare ustioni.
...
\begin{itemize}
  ...
  \item $V_L (t) = V_R (t) = i_0 \cdot R \cdot
 e^{-t/\tau}$
\end{itemize}
...
dove $s$ è una variabile complessa, e abbiamo ipotizzato che la corrente segua l'evoluzione $i(t,s) = e^{st}$ per la resistenza e per l'induttore e che la tensione segua l'evoluzione $v(t,s) = e^{st}$ per il condensatore.
...
Se $s = j\omega \Rightarrow e^{st} = \cos (\omega t) + j\sin (\omega t) \Rightarrow |e^{st}| = 1 \;\, \forall t$
