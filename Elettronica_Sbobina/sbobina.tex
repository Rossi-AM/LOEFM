...
\[P_R (t) = V_R (t) * I_R (t) \quad \textrm{oppure} \quad P_R (t) = V_R^2 (t) / R\]
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_condensatore}
  \caption{Schizzo di un condensatore}
  \label{Schema_condensatore}
\end{figure}

Un condensatore immagazzina al suo interno una carica $Q$, misurata in coulomb (C). Questa non è che l'integrale della corrente che attraversa il condensatore nel tempo.
...
Un condensatore è in grado di immagazzinare energia
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_induttore}
...
\begin{itemize}
  ...
  \item $\frac{\textrm d V_C (t)}{\textrm d t} = \frac{I_C (t)}{C} = - \frac{I_R (t)}{C} = -\frac{V_R (t)}{C * R} = -\frac{V_C (t)}{C * R}$
  \item $V_C (t) = V_R (t) = v_0 * e^{-t/(CR)}$
  \item $I_C (t) = -I_R (t) = \frac{v_0}{R} * e^{-t/(CR)}$
\end{itemize}
...
\begin{itemize}
  ...
  \item $I_L (t) = -I_R (t) = i_0 * e^{-t/\tau}$
  \item $V_L (t) = V_R (t) = i_0 * R * e^{-t/\tau}$
\end{itemize}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_kirchhoff_nodi}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.5]{IM_kirchhoff_rami}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.5]{IM_kirchhoff_maglie}
...
\[V_{R1} = I * R_1 \quad \textrm{e} \quad V_{R2} = I * R_2\]

\[V_{R1} + V_{R2} = V_0\]

quindi

\[V_0 = I * (R_1 + R_2) \Rightarrow I = \frac{V_0}{R_1 + R_2} = \frac{V_0}{R_{eq}}\]

\[R_{eq} = R_1 + R_2\]

Se ho due o più resistenze collgate in serie, esse equivalgono ad un unico resistore la cui resistenza equivalente è pari alla somma delle singole resistenze.

Se si volesse ora trovare il valore di $V_{R1}$ basta pensare che

\[V_{R1} = I * R_1 = V_0 \frac{R_1}{R_{eq}}\]
...
\[V = I_1 * R_1 \quad \textrm{e} \quad V = I_2 * R_2\]
...
Se ho due o più resistenze collgate in parallelo, esse equivalgono ad un unico resistore la cui resistenza equivalente è pari alla somma armonica delle singole resistenze.
...
\begin{itemize}
  ...
  \item $P_{OUT}$ (generata) $= V_{OUT} * I_{OUT}$
\end{itemize}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_Thevenin_Norton_ter}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_Thevenin_Norton_quater}
  \caption{Modelli equivalenti di una generica rete LTI}
...
Se ci si aspetta che un circuito sia un generatore di tensione, sarà ragionevolmente sicuro misurare $V_{eq}$ (ma non $I_{eq}$). Viceversa, se ci si aspetta che sia un generatore di corrente, sarà ragionevolmente sicuro misurare $I_{eq}$ (ma non $V_{eq}$). Leggere sempre e comunque le istruzioni d'uso prima di fare qualunque cosa.
...
Esempio: il caricatore del cellulare (USB) è equivalente ad un generatore di tensione (reale), con differenza di potenziale in uscita pari a $V_{OUT} = +5$V e corrente in uscita $I_{OUT} < 1$A.
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_impedenze}
  \caption{Impedenze di sorgente e di cariche}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_effetto_di_carico}
...
dove $s$ è una variabile complessa, e abbiamo ipotizzato che la corrente segua l'evoluzione $i(t,s) = e^{s*t}$ per la resistenza e per l'induttore e che la tensione segua l'evoluzione $v(t,s) = e^{s*t}$ per il condensatore.

Otteniamo così l'\textit{impedenza} Z, che si misura in ohm, e l'\textit{ammettenza} Y, che si misura in siemens. Queste due grandezza sono legate dalla relazione Z$=\textrm{Y}^{-1}$. Essendo tuttavia il paraetro $s$ complesso, anche le grandezze Z e Y sono complesse.
...
\[e^{st}= e^{(\sigma + j*2\pi f)t}= e^{\sigma t}(\textrm{cos} (2\pi f * t) + j\textrm{sin} (2\pi f * t))\]

Se $s = j\omega \Rightarrow e^{st} = \textrm{cos} (\omega t) + j\textrm{sin} (\omega t) \Rightarrow |e^{st}| = 1 \;\, \forall t$
...
\[f(C_1 * IN_1 + C_2 * IN_2) = C_1 * f(IN_1) + C_2 * f(IN_2)\]

$f$ è un sistema tempo invariante se e solo se le sue proprietà non dipendono dal tempo.

È detta funzione di trasferimento (FdT) la relazione (H), espressa nei domini trasformati di Laplace (s) o di Fourier (f, $\omega$), tra un ingresso e un’uscita di $f$.
...
Nel caso del secondo circuito ho collegato al doppio dipolo un voltmetro ideale, a impedenza infinita, che misura la differenza di potenziale ai capi della resistenza $R_2$. La FdT H(s) è il rapporto di partiziobne tra la resistenza $R_1$ e la resistenza $R_2$.

\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_complesso}
  \caption{Un circuito complesso, modellizzato in 3 blocchi in cascata}
...
In assenza di effetti di carico (o se le singole Funzioni di Trasferimento sono ricavate considerano le impedenze di sorgente e di carico), la FdT complessiva corrisponde al prodotto delle singole FdT.
...
Carico il condensatore con un generatore di corrente che ha un andamento impulsivo, quindi la corrente emessa dal generatore ha valore 0 per ogni $t$ diverso da 0. Al tempo $t=0$ il generatore emette un impulso di corrente che carica il condensatore di una carica $q_0$ . 
...
\[in(t) = \int_{-\inf}^{\inf} in(\xi) \cdot \delta (t - \xi) \cdot \textrm{d} \xi = \int_{-\inf}^{\inf} IN (f) \cdot e^{j \omega f t} \cdot \textrm d f\]
...
Definiamo la \textit{traformata di Fourier} ($\mathcal{F}$) e la sua \textit{anti-trasformata} ($\mathcal{F}^{-1}$)

\[IN (f) = \mathcal{F} (in(t)) = \int_{-\inf}^{\inf} in(t) \cdot e^{-j2\pi ft} \cdot \textrm d t\]

\[in (t) = \mathcal{F}^{-1} (IN(f)) = \int_{-\inf}^{\inf} IN(t) \cdot e^{j2\pi ft} \cdot \textrm d t\]
...
Cominciamo con il circuito RC (figura \ref{Schema_circuito_RC_passivo}):
...
\[V_{OUT}(s) = V_{IN}(s) * \frac{Z_C(s)}{Z_C(s) + Z_R(s)}\]
...
\[V_{OUT}(s) = V_{IN}(s) * \frac{Z_L(s)}{Z_L(s) + Z_R(s)}\]
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_LR_passivo}
  \caption{Circuito LR}
  \label{Schema_circuito_LR_passivo}
\end{figure}

\[V_{OUT}(s) = V_{IN}(s) * \frac{Z_R(s)}{Z_L(s) + Z_R(s)}\]
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_risonante_serie_LC}
  \caption{Circuito risonante serie LC}
  \label{Schema_circuito_risonante_serie_LC}
\end{figure}

\[V_{OUT}(s) = V_{IN}(s) * \frac{Z_C(s)}{Z_L(s) + Z_C(s)}\]
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_risonante_serie_LC_grafici}
  \caption{La funzione di trasferimento al variare della pulsazione}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_risonante_serie_CL}
  \caption{Circuito risonante serie CL}
  \label{Schema_circuito_risonante_serie_CL}
\end{figure}

\[V_{OUT}(s) = V_{IN}(s) * \frac{Z_L(s)}{Z_L(s) + Z_C(s)}\]
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_risonante_serie_CL_grafici}
  \caption{La funzione di trasferimento al variare della pulsazione}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_risonante_serie_RLC}
  \caption{Circuito risonante serie RLC}
  \label{Schema_circuito_risonante_serie_RLC}
\end{figure}

\[V_{OUT}(s) = V_{IN}(s) * \frac{Z_C(s)}{Z_R(s) + Z_L(s) + Z_C(s)}\]
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_risonante_serie_RLC_grafici}
  \caption{La funzione di trasferimento al variare della pulsazione}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_risonante_parallelo_LC}
  \caption{Circuito risonante parallelo LC}
  \label{Schema_circuito_risonante_parallelo_LC}
\end{figure}

\[V_{OUT}(s) = I_{IN}(s) * (Z_C(s) // Z_L (s))\]
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_risonante_parallelo_LC_grafici}
  \caption{La funzione di trasferimento al variare della pulsazione}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_risonante_parallelo_RLC}
  \caption{Circuito risonante parallelo RLC}
  \label{Schema_circuito_risonante_parallelo_RLC}
\end{figure}

\[V_{OUT}(s) = I_{IN}(s) * (Z_C(s) // Z_L (s) // Z_R (s))\]
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_risonante_parallelo_RLC_grafici}
  \caption{La funzione di trasferimento al variare della pulsazione}
...
Un amplificatore operazionale è un dispositivo ad almeno 5 terminali (anche se molto spesso ne hanno in numero maggiore). Questi 5 terminali sono così suddivisi:
...
un'uscita di tensione; due terminali di alimentazione.
...
\begin{itemize}
  ...
  \item Uscirta di tensione a impedenza nulla (generatore di tensione ideale)
  \item Alimentazione con differenza di potenziale infinita
  \item Modellizzabile con un sistema lineare:
  \[V_{OUT} = f(V_{IN+} - V_{IN-}) = f(\Delta V_{IN}) = A_0 * \Delta V_{IN} \quad (A_0 \textrm{ cost.} \rightarrow \infty)\]
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_amplificatore_operazionale_modello}
  \caption{L'invalidità del modello}
  \label{Schema_amplificatore_operazionale_modello}
\end{figure}

No. Cambiando il potenziale di riferimento non cambio la differenza di potenziale tra i due ingressi, quindi il potenziale in uscita resta invariato. Ma questo, come visibile dai conti in figura \ref{Schema_amplificatore_operazionale_modello}, è un paradosso.

\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_amplificatore_operazionale_modello_bis}
  \caption{Un modello alternativo}
  \label{Schema_amplificatore_operazionale_modello_bis}
\end{figure}

Il modello altrenativo schematizzato in figura \ref{Schema_amplificatore_operazionale_modello_bis}, invece, resta valido al variare del potenziale di riferimento, ma non è realistico in quanto prevede un ulteriore terminale che l'amplificatore operazionale non ha.

\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_amplificatore_operazionale_meno_ideale}
...
\begin{itemize}
  ...
  con $\Delta V_{CC}$ e $\Delta V_{EE}$ compresi tra 0 e qualche Volt a seconda del tipo di Op. Amp.
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_amplificatore_operazionale_grafico}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_amplificatore_operazionale_carta_e_penna}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_amplificatore_operazionale_reale}
  \caption{Modello di amplificatore operazionale più realistico, che funziona sempre , ma è anche più complicato...}
...
\[V_{OUT} = V_- - V_{R2} = - V_{R2} = - I_{R2} * R_2 = - I_{R1} * R_{2} = - V_{IN} * \frac{R_{2}}{R_{1}}\]
...
\[V_{OUT} = V_{R1} + V_{R2} = V_{IN} + V_{IN} * \frac{R_2}{R_1}\]
...
Il circuito raffigurato in figura \ref{Schema_circuito_sommatore_invertente} è un circuito con più ingressi. È possibile analizzarlo, per il principio di sovrapposizione, ``spegnendo'' un ingresso ed analizzando l'effetto dell'altro, spegnendo quindi il secondo e analizzando il primo e sommare quanto trovato.
...
\[V_{OUT2} = - V_{IN2} \frac{R_2}{R_12}\]

Analogamente, spegnendo l'ingresso 2, si ha che

\[V_{OUT1} = - V_{IN1} \frac{R_2}{R_11}\]

e infine

\[V_{OUT} = V_{OUT1} + V_{OUT2} = - V_{IN1} \frac{R_2}{R_11} - V_{IN2} \frac{R_2}{R_12} = - R_2 * \left( \frac{V_{IN1}}{R_{11}} + \frac{V_{IN2}}{R_{12}} \right)\]

facilmente generalizzabile a 

\[V_{OUT} = - R_2 * \left( \frac{V_{IN1}}{R_{11}} + \frac{V_{IN2}}{R_{12}} + \frac{V_{IN3}}{R_{13}} + ... \right)\]
...
\[V_+ = V_{IN2} * \frac{R_4}{R_3 + R_4}\]

adesso la configurazione si è ridotta ad un normale non invertente, per cui

\[V_{OUT2} = V_+ + V_+ * \frac{R_2}{R_1} = V_+ * \frac{R_2 + R_1}{R_1} = V_{IN2} * \frac{R_4}{R_3 + R_4} * \frac{R_2 + R_1}{R_1}\]

\[V_{OUT} = - V_{IN1} * \frac{R_2}{R_1} + V_{IN2} * \frac{R_4}{R_3 + R_4} * \frac{R_2 + R_1}{R_1}\]

caso particolare, se $R_4 / R_3 = R_2 / R_1$,

\[V_{OUT} = (V_{IN2} - V_{IN1}) * \frac{R_2}{R_1}\]
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_filtro_integratore}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_filtro_integratore_grafici}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_filtro_passa_alto_CR_attivo_bis}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_filtro_passa_alto_CR_attivo_grafici}
...
\[T_{HP} = \frac{1}{\omega _{HP}} = C_1 R_1 = 3 m \textrm s\]
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_filtro_derivatore}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_filtro_derivatore_grafici}
...
Iniziamo a parlare di circuiti risonanti attivi con il circuito in figura \ref{Schema_circuito_risonante_serie_attivo}. 
...
Si nota immediatamente che non ci sono segnali in ingresso al circuito. Questo produce autonomamente il segnale $V_{OUT}$ in uscita. Il circuito può idealmente essere suddiviso in due parti:
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_risonante_parallelo_attivo_3}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_giratore}
  \caption{Circuito giratore}
  \label{Schema_circuito_giratore}
\end{figure}

Il circuito giratore, schematizzato in figura \ref{Schema_circuito_giratore}, permette di “emulare” un induttore a partire da un condensatore. Spesso usato nei circuiti integrati, dove gli induttori fisici non sono disponibili.
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_giratore_grafico}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_diodo_p_n}
  \caption{Diodo a giunzione p-n in silicio, con simbolo circuitale e grafico della tensione}
  \label{Schema_diodo_p_n}
\end{figure}

Un diodo è una giunzione formata da due pezzi di uno stesso materiale (in questo caso silicio) contatto tra di loro, ``drogati'' in maniera diversa (di solito Boro e Fosforo). Questi drogaggi portano uno ad una eccedenza di elettroni e l'altro ad una carenza di elettroni.
...
per denominatore il prodotto tra una costante numerica $\alpha$ (di valore compreso tra 1 e 2) ed una quantità detta \textit{tensione termica} (che vale $V_T = \frac{kT}{q}$ dove $k$ è la costante di Boltzmann, $T$ la temperatura, $q$ la carica dell'elettrone; a temperatura ambiene vale circa $25$ mV).

Il fattore $-1$ in sottrazione serve per fare sì che la corrente sia nulla a tensione nulla.

Per tensioni negative, la corrente è (quasi) uniformemente pari alla corrente di saturazione (che è inaspettatamente l'inverso della corrente di saturazoine inversa).
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_diodo_resistenza_equivalente}
...
In alternativa, più semplicemente, si può utilizzare un amplificatore operazionale in configurazione di amplificatore invertente, per realizzare un generatore di corrente equivalente controllato in 
tensione. Basta quindi poi usare un voltmetro per misurare, indirettamente, la differenza di potenziale ai capi del diodo.

\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_diodo_misura_ter}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_rettificatore_half_wave}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_rettificatore_half_wave_2}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_rettificatore_full_wave}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_rettificatore_half_wave_c}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_rettificatore_full_wave_c}
...
\[\textrm{Se } V_O = +V_{CC} \textrm{ il diodo è spento } I_D = 0 = I_{R1} \Rightarrow V_{OUT} = 0 \; \; V_O = A_O \cdot (V_+ - V_-) = A_O \cdot V_{IN} = -V_{EE}\]

\[\textrm{Se } V_O = -V_{EE} \textrm{ il diodo è acceso } V_{OUT} = V_O + V_{TH} = -V_{EE} + 0.6 \; \; V_O = A_O \cdot (V_{IN} + V_{EE} - 0.6) = +V_{CC}\]
...
\[\textrm{Se } V_O = +V_{CC} \textrm{ il diodo è spento } I_D = 0 = I_{R1} \Rightarrow V_{OUT} = 0 \; \; V_O = A_O \cdot (V_+ - V_-) = A_O \cdot V_{IN} = +V_{CC}\]
...
Si noti che, da questo momento in poi, considerato che il funzionamento di molti circuiti dipende dai valori degli elementi costruttivi, la teoria generale non verrà più presentata al prescindere di un circuito esemplare che sicuramente funziona. In laboratorio è saggio creare circuiti con pezzi il più possibile vicini a quelli indicati negli esempi, per evitare di trovarsi impossibilitati ad osservare il comportamento tipico di un circuito, altrimenti ben assemblato, a causa di un pezzo anomalo. In breve, copiate le figure.
...
Le correnti $I_{R3}$ e $I_C$ cambiano verso. $V_{OUT1}$ e $V_X$ crescono ora linearmente nel tempo. Quando $V_X$ diventa positiva, 

\[V_{OUT2} = + V_{CC} = + 10 \textrm V\]

ritornando al punto di partenza. Il ciclo continua indefinitamente. In particolare $V_{OUT2}$ commuta periodicamente tra i valori $+V_{CC}$ e $-V_{EE}$, ed è quindi un'\textit{onda quadra}. Il segnale $V_{OUT1}$, invece, varia linearmente, crescendo quando $V_{OUT2}$ è negativo e decerescendo quando $V_{OUT2}$ è positivo (\textit{onda triangolare}). I limiti del segnale $V_{OUT1}$ si calcolano semplicemente imponendo $V_X = 0$

\[0 = V_X = V_{OUT1} \frac{R_2}{R_1 + R_2} + V_{OUT2} \frac{R_1}{R_1 + R_2} = 0.9 \cdot V_{OUT1} + 0.1 \cdot V_{OUT2} = 0.9 \cdot V_{OUT1} \pm 1 \textrm V\]

\[V_{OUT1} = \pm 1.11 \textrm V\]

Il periodo del segnale ad onda quadra corrisponde al tempo necessario perchè il condensatore $C$ inverta la sua carica:
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_circuito_astabile_bis}
...
Procediamo con l'analisi: per $V_Y < V_{TH} (\approx 0.6 \textrm V)$ il comportamento del circuito sarà quindi identico a quello del precedente in quanto il diodo, non conducendo corrente, non altererà il funzionmento del circuito. Il circuito potrà quindi liberamente commutare dallo stato $V_{OUT} = - V_{EE}$ allo stato $V_{OUT} = + V_{CC}$. Una volta raggiunto lo stato $V_{OUT} = + V_{CC}$, $V_X$ passa immediatamente al valore di $1$V, ma il potenziale $V_Y$ è impossibilitato a superare il valore $V_{TH} \approx 0.6$ dalla presenza del diodo. 

Il circuito raggiunge quindi uno stato stabile, non presente nei precedenti circuiti astabili (come intuibile dai nomi stessi). Una volta portato nello stato instabile, il circuito tornerà nello stato stabile in un tempo caratteristico determinato dai suoi componenti. Per ``spingere” il circuito monostabile fuori dallo stato stabile occorre intervenire dall’esterno (come fornendo una spinta impulsiva), attraverso diversi possibili circuiti. Nell'esempio in figura \ref{Schema_circuito_monostabile_bis} si usa un sistema di tre componenti.
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_dispositivo_attivo_base}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{Triode_vacuum_tube}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_JFET}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_MOS_simboli}
...
\begin{figure}[h]
  ...
  \caption{MOS in funzoine (si noti il canale più stretto nei pressi del Drain)}
...
dove $\mu _n$ è la mobilità relativa degli elettroni nel materiale semiconduttore, $C_{ox}$ è la capacità, per elemento di superficie, del ``condensatore'' metallo-ossido-semiconduttore, $W$ è la larghezza (width) del canale e $L$ la sua lunghezza (length). Si può vedere che (al netto del termine perturbativo $V_{DS}^2$, che possiamo trascurare in quanto termine del secondo ordine nell'ipotesi in cui $V_{DS}$ sia piccolo rispetto a $V_{GS} - V_{TH}$) la corrente dipende linearmente da $V_{DS}$. 
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_Effetto_Early}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_IGBT}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_determinazione_MOS_bis}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_MOS_grafici_bis}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_MOS_equivalente_saturazione}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_MOS_equivalente_saturazione_bis}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_MOS_equivalente_lineare}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_MOS_equivalente_lineare_bis}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_inverter}
  \caption{Circuito con carico resistivo}
  \label{Schema_inverter}
\end{figure}

Si consideri adesso il circuito in figura \ref{Schema_inverter}, e in particolare alla relazione tra $V_{IN}$ e $V_{OUT}$. Per $V_{IN}$ piccolo il MOS è spento, in quanto il potenziale di Gate è insufficiente perché si formi il canale e ci sia il relativo passaggio di corrente. Senza passaggio di corrente, per la legge di Ohm non c'è differenza di potenziale ai capi della resistenza, e $V_{OUT} = + V_{CC}$. Se $V_{IN}$ è sufficientemente alta, invece, il MOS è acceso e la corrente vi scorre liberamente all'interno. $V_{OUT}$ è equipotenziale (o quasi) con la terra. $V_{OUT} \approx 0$.

\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_inverter_bis}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_inverter_ter}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_inverter_quater}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_inverter_penta}
  \caption{Dissipazione di potenza}
  \label{Schema_inverter_penta}
\end{figure}

Questo circuito rappresenta un'ottima occasione per iniziare a discutere di circuiti digitali e della loro differenza dai circuiti analogici. In ambito analogico i segnali sono continui (dominio e codominio), come lo è ad esempio il segnale elettrico prodotto da un microfono. In ambito digitale i segnali sono discreti (dominio e codominio) o ``sequenze quantizzate'', esprimibili con combinazioni di variabili elementari (bit) a due stati (``0/1'', ``alto/basso'', ecc.). A livello macroscopico, però, il mondo è continuo in tempo e ampiezza. In un dispositivo digitale, quindi, i segnali ``portano informazione'' soltanto in predeterminati istanti di tempo e l’informazione elementare (bit) è associata ed estrapolabile dal valore del segnale di tensione (o di corrente).
...
Sostituendo il carico resistivo (passivo) con un dispositivo attivo complementare (MOS a canale P) la dissipazione di potenza è nulla in entrambi gli stati (alto e basso) essendo uno tra i due MOS sempre spento ()Sostituendo il carico resistivo (passivo)
con un dispositivo attivo complementare 
(MOS a canale P) la dissipazione di potenza è nulla 
in entrambi gli stati (alto e basso) essendo 
uno tra i due MOS sempre spento
• Questa logica è detta CMOS (Complementary MOS)
• In logica CMOS la relazione ingresso/uscita è 
solitamente resa simmetrica (figura \ref{Schema_CMOS}). Questa logica è detta CMOS (Complementary MOS). In logica CMOS la relazione ingresso/uscita è solitamente resa simmetrica.
...
Dove $S$ è l'insieme su cui sono definite le operazioni, $+$ è l'operazione di OR, $*$ è l'operazione di AND, $!$ è l'operazione di NOT, 0 è l'elemento neutro di $+$ e 1 l'elemento neutro di $*$. In particolare, nell'algebra Booleana a due elementi (algebra di commutazione), $S = \{0, 1\}$. Le variabili 1 e 0 vengono chiamate anche, rispettivamente, \textit{vero} e \textit{falso}.
...
Ogni funzoine logica combinatoria può essere espressa tramite l'uso dei tre operatori: AND, OR, NOT.
...
Se i due ingressi sono identici, la porta si riduce ad un invertitore (NAND(A, A) = NOT(A)).

Se adesso invertiamo la porta NAND, otteniamo una porta AND (NOT(NAND(A, B)) = AND(A, B)).

Se adesso invertiamo gli ingressi alla porta NAND, otteniamo una porta OR (NAND(A', B') = NOT(AND(A', B')) = NOT(NOT(OR(A, B))) = OR(A, B)).

È quindi possibile realizzare qualsiasi funzione logica combinatoria anche utilizzando esclusivamente porte NAND. La porta NAND possiede quindi la proprietà di completezza funzionale: ogni altra funzione logica (AND, OR etc.) può essere implementata utilizzando solamente porte NAND (figura \ref{NAND_allpowerful}).
...
Una variabile booleana (un bit) è in grado di rappresentare due cose diverse: 0 e 1. Questa quantità di informazione è quasi sempre insufficiente nelle applicazioni quotidiane dei dispositivi digitali, ma è possibile risolvere questo problema affiancando un grande numero di bit.
...
Dove $A_b$ è una cifra binaria (cioè un bit). Alcuni esempi:
...
Esistono diversi modi per realizzare un sommatore a $B$ bit. Il primo metodo è quello della sintesi diretta (figura \label{grafici_sommatori}, in alto a destra): ognuna delle $B+1$ funzioni logiche rappresentanti le uscite può essere scritta in forma canonica (ad esempio come somma di prodotti) a partire dalle $2^2B$ combinazioni degli ingressi. In questo modo servirebbero circa:
...
Il tempo richiesto per il calcolo non dipenderebbe da B, ma l’implementazione è fattibile solo per $B$ piccolo. Esistono implementazioni più semplici di questo sommatore, anche se meno performanti. Il meno costoso, anche se meno efficiente, è il ripple carry adder.

Immaginiamo di sommare due numeri binari in colonna:

Iniziando dalla cifra meno significativa, possiamo eseguire l'operazoine di somma con un semplice confronto:
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_full_adder}
  \caption{Full adder}
  \label{full_adder}
\end{figure}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_full_adder_bis}
  \caption{Full adder più economico}
  \label{full_adder_bis}
\end{figure}

Un'implementazione potrebbe essere come raffigurata in figura \ref{full_adder}. Si possono risparmiare porte logiche ricordando che 
...
La memoria a più celle (figura \ref{memoria_piu_celle}) è formata da $B_I$ memeorie a una cella da $B_D$ bit ciascuna. Per accedere ad una specifica cella di memoria serve conoscerne l'indirizzo. Esistono memorie con poche celle, ad esempio $N = 256$ e $B_I = 8$ linee di indirizzo, fino a centinaia di milioni di celle e molte decine di linee di indirizzo.
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_flip_flop_NOR}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_flip_flop_NAND}
...
\begin{figure}[h]
  \centering
  %\captionsetup{justification=centering,margin=2cm}
  \includegraphics[scale=0.7]{IM_FPGA_struttura}
